-- Make session discussion policies more permissive
-- Allow any authenticated user to create and participate in session discussions

-- Update discussions policies to be more permissive for sessions
...
CREATE POLICY "Users can view discussions they participate in" ON discussions
  FOR SELECT USING (
    ...
    (discussion_type = 'session' AND auth.uid() IS NOT NULL)
  );
...
-- Update discussion_participants policies to be more permissive for sessions
...
-- Update posts policies to be more permissive for sessions
...
-- Update replies policies to be more permissive for sessions
...
CREATE POLICY "Users can view replies in their discussions" ON replies
  FOR SELECT USING (
    post_id IN (
      ...
      (d.discussion_type = 'session' AND auth.uid() IS NOT NULL)
    )
  );
...
-- Update post_reactions policies to be more permissive for sessions
...
-- Update reply_reactions policies to be more permissive for sessions
